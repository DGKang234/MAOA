...
class ML_train_generator(extractor):
    ...
    @property
    def breathing(self):
        Lambda = len(np.arange(0.8, 1+self.step_size, self.step_size)) #* self.no_atoms*3
        ...
        for numj, j in enumerate(np.arange(0.8, 1+self.step_size, self.step_size)):
            j = np.round(j, 2)
            frame = coord * j
            self.mod_sp_breath[cnt] = np.round(frame, 8)
            cnt += 1

        self.mod_sp_breath = np.reshape(self.mod_sp_breath, (len(np.arange(0.8, 1+self.step_size, self.step_size)), self.no_atoms, 3)) 
        self.breathing_called = True
        return self.mod_sp_breath
    ...
    def geometry_for_sp(self):
        ...
        # breathing mode
        if self.breathing_called: 
            placer_breath = np.full((self.no_atoms, 1), 'atom')
            placer_species_breath = np.reshape(self.order, (-1, 1))
            shape_breath = np.shape(self.mod_sp_breath)
            self.for_sp_breath = np.empty((shape_breath[0], shape_breath[1], self.no_atoms, 5), dtype=object)
            print(shape_breath)
            for i in range(shape_breath[0]):
                for j in range(shape_breath[1]):
                    form = np.concatenate((placer_breath, self.mod_sp_breath[i][j], placer_species_breath), axis=1)
                    self.for_sp_breath[i][j] = form 
            return self.for_sp, self.no_atoms, self.for_sp_breath
        else: 
...
if __name__ == "__main__":
    ...
    no_atoms = np.shape(ml.geometries)[1]
    ...
    if args.mode == "run":
        ...
        ml.mod_xyz_w_vib
        #ml.breathing
        #sp_frame, no_atoms, sp_frame_breath = ml.geometry_for_sp
        sp_frame, no_atoms = ml.geometry_for_sp
